...
public class SelfJoinHandler implements ExpressionVisitor  {
	...
	public AccessArea handleSelfJoins(AccessArea accessArea){
		...
		//Iterate through list of relations, add to TreeMap, see if duplicate (don't add in that case)
		//If duplicate was found, then remove one occurrence, go through the constraints, search for its alias and replace on every column with name of the relation 
		while(it.hasNext()){
			Table table = (Table)it.next();
				...
				if(tmPred.containsKey(alias1)){
					it.remove();
					if(!selfJoinS.contains(alias1)) selfJoinS.add(alias1);
				} else {
					where.accept(this);
					where = stack.pop();
				}
		...
		firstRun = false; //now we have to go through the epxression again and whenever a table that was part of a self join is addressed, we replace it with our new expression of self-join predicates
	...
	//Whenever we reach a column we have to check to which table and or alias it belongs, replace the alias if its part of a self join and store the predicate
	public void processColumn(Column tableColumn){
		Expression expression = tableColumn;
		String column = tableColumn.getTable().toString();
		if(firstRun){//replace alias on any column by name of relation (if it is part of a self-join)
			...
			if (column.equals(alias2)){
				...
				if(!tmPred.containsKey(alias1)) tmPred.put(alias1,tempExpression);
				else {
		...
		} else if (column.equals(alias1)) {//in second run we replace any self-join expression with our new set of predicates and in any clause we do this only once. all other self-join predicates are removed from the clause
	...
	public void visitBinaryExpression(BinaryExpression binaryExpression) {
		...
		if(stack.peek() instanceof AndExpression && foundAlias) foundAlias = false;//Start fresh for every new clause

		//Only traverse ANDs, ORs or direct children. we do not need to get deeper into the hierarchy (e.g., no formulas)
		if(tempRightExpression instanceof BinaryExpression && (stack.peek() instanceof AndExpression || stack.peek() instanceof OrExpression))tempRightExpression.accept(this);
		else if(tempRightExpression instanceof Column) {
			if (((Column)tempRightExpression).getTable().toString().equals(alias1) || ((Column)tempRightExpression).getTable().toString().equals(alias2)) ((Column)tempRightExpression).setTable(new Table(null,alias1));
		...
		if(parent != null){//If any predicate was deleted handle it accordingly, make a nice correct structure
			if(left == null) parent = right;
			else {
				if(left instanceof OrExpression && parent instanceof AndExpression) left = new Parenthesis(left); 
				if(right instanceof OrExpression && parent instanceof AndExpression) right = new Parenthesis(right); 
	...
	public void visit(IsNullExpression isNullExpression) {		
		...
		if(stack.pop() == null) isNullExpression = null;
		stack.push(isNullExpression);
	}
