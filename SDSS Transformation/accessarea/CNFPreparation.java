...
public class CNFPreparation implements ExpressionVisitor  {
	...
	//We traverse the constraint once, store every predicate in a numbered list and replace it in the where expression by P(px), e.g. P(p1)
	public PreparedPredicates prepareForCNF(Expression predicate){
		...
		//for constructing our final CNF constraints we need the list of original predicates and the new expression with P(px)
		preparedPredicates.setExpressions(predicateList);
		
		 //Currently CNF Converter get's stuck with more than 35 predicates in one access area
		//If you want to use our method of limiting any larger access area to the first 35 predicates remove the following line
		if(predicateCounter > 35) return null;
		
		//the following while loop searches the predicate with the currently highest number and removes it.
		//This is done until only 35 predicates are left. The number can be changed of course, only a parameter
	...
	//Here we replace predicates with P(px), save the original predicate in our list of expressions and increment the counter
	...
	//in normal mode we only traverse the right hand side and left hand side of each binary expression
	//if we are in "tooMany" mode, i.e. more than 35 (or whatever) predicates are in the current access area, then we catch the one with the largest number and remove it.
	//this is done here since we cannot change the structure inside the visit methods
	public void visitBinaryExpression(BinaryExpression binaryExpression) {
		...
		if(tooMany) {//Dont care about parenthesis, are only searching for predicate with highest number (i.e. it's name is now "remove(px)"
			...
			if(tempLeftExpression instanceof BinaryExpression) {
				...
				if(tempLeftLeftExpression instanceof Parenthesis) tempLeftLeftExpression =  ((Parenthesis)tempLeftLeftExpression).getExpression();
				if(tempLeftRightExpression instanceof Parenthesis) tempLeftRightExpression =  ((Parenthesis)tempLeftRightExpression).getExpression();
			...
			if(tempRightExpression instanceof BinaryExpression) {
				...
				if(tempRightLeftExpression instanceof Parenthesis) tempRightLeftExpression =  ((Parenthesis)tempRightLeftExpression).getExpression();
				if(tempRightRightExpression instanceof Parenthesis) tempRightRightExpression =  ((Parenthesis)tempRightRightExpression).getExpression();
	...
	@Override
	public void visit(Function function) { //should not occur anymore in normal more, but occurs in tooMany Mode
		//if predicate count >35 we rename the function with the highest number to 'remove(px)'
		//due to the tree structure of different types we cannot remove it here,  do it in the visitBinaryExpression method
		if(tooMany) {
			if(((Column)function.getParameters().getExpressions().get(0)).getWholeColumnName().equals("p"+(predicateCounter-1))&&predicateCounter>35) {
				predicateCounter--;
				function.setName("remove");
			}
		} else	processExpression(function);
	}
	...
	@Override //Usually just traverse the structure, in tooMany Mode we remove any function with the name 'remove(px)'
		...
		if(tooMany) {
			...
			if(parenthesis.getExpression() instanceof BinaryExpression) {
				if(((BinaryExpression)parenthesis.getExpression()).getLeftExpression() instanceof Function) {
					if((((Function)((BinaryExpression)parenthesis.getExpression()).getLeftExpression()).getName().equals("remove"))){
						parenthesis.setExpression(((BinaryExpression)parenthesis.getExpression()).getRightExpression());
					}
				}
			}
			if(parenthesis.getExpression() instanceof BinaryExpression) {
				if(((BinaryExpression)parenthesis.getExpression()).getRightExpression() instanceof Function) {
					if((((Function)((BinaryExpression)parenthesis.getExpression()).getRightExpression()).getName().equals("remove"))){
	...
	public void visit(AndExpression andExpression) {
		...
		} else if (predicateCounter > 35) { //try to limit it even before, does not work as expected, problem due to arbitrary structure of queries, i.e., arbitrary tree hierarchy
			if(andExpression.getLeftExpression() instanceof AndExpression)
				andExpression.getRightExpression().accept(this);
			else andExpression.getLeftExpression().accept(this);
		} else {
	...
	public void visit(OrExpression orExpression) {
		if(tooMany) {
			...
			else orExpression.getLeftExpression().accept(this);
		} else {
