...
public class FinalCNF  {
	...
	public Expression getCNFPredicates(CNF cnf, List<Expression> expressions){
		...
		if(clauses == 0) stack.push(null);
		else if(clauses == 1 && expressions.size() == 1) stack.push(expressions.get(0));
		else {
			for(int i = 0; i < clauses; i++) {//For each clause process the literals (i.e. create ORs) and for each clause (if more than one clauses) create an AND
				...
				for(int j = 0; j < literals; j++) {//Create as many ORs as necessary (i.e. count of literals) and fill one side with the current expression
					...
					} else stack.push(expressions.get(id));
				}
				while(true) { //connect the ORs until stack is empty or an AND is reached
				...
				if(literals != 1) {//Create parenthesis if more than 1 literals
					parenthesis = new Parenthesis(stack.pop());
				} else {
					parenthesis = stack.pop();
				}
				if(i < clauses-1 && clauses != 1) {//If more than one clause exists create a new AND
