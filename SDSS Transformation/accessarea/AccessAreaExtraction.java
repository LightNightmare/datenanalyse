...
public class AccessAreaExtraction {
	...
	public AccessArea extractAccessArea(String stmt) {
		...
		//The following cleans the SQL statement from any typical SDSS String errors
		stmt = correctSDSSLogErrors(stmt); //Remove this line if used on DB other than SDSS and if the corrections could clash with log contents
		
		//Parse the Query with the jSQL parser as a SELECT statement and extract the relations and predicates (calls the Converter class)
		try {
			//long startTime = new Date().getTime();
			accessArea = parser.parseStmt(stmt);
			//System.out.println("Transform: " + ((new Date().getTime())-startTime) + " milliseconds");
		...
		//Pass the access area through the subsequent steps: CNF preparation, conversion, self-join-handling and consolidation
		try {
			if(accessArea.getWhere() != null) {
				if(!(accessArea.getWhere() instanceof NullValue || accessArea.getWhere() instanceof BooleanValue)) { 
					...
					folDomain.addPredicate("P"); //Our own syntax for cnf conversion
					...
					if(preparedAccessArea == null) return null; //Something went wrong, usually does not happen anymore
					
					//if(preparedAccessArea.getWhere().toString().contains("remove(p")) return null; //Was used when limiting to 35 predicates, did not always work as expected
					...
					if(cnf != null)	accessArea.setWhere(finalCNF.getCNFPredicates(cnf, preparedAccessArea.getExpressions()));
		...
		try { //If there are relations we remove any alias and sort them alphabetically
	...
	private String correctSDSSLogErrors(String stmt) {
		...
		//if(stmt.toLowerCase().startsWith("set parseonly on")){//solved further below
		...
		stmt = stmt.replace("SELECT", " SELECT ");
		stmt = stmt.replace("Select", " Select ");
		stmt = stmt.replace("from ", " from ");	//Functions do not end with from
		stmt = stmt.replace("FROM ", " FROM "); //only < 8000 where FROM occurs two times -> subqueries or comments, not used inside special names like functions
		stmt = stmt.replace("From ", " From "); //Functions do not end with from
