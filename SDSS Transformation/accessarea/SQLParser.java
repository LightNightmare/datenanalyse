...
public class SQLParser implements SelectItemVisitor,ExpressionVisitor, SelectVisitor { //FromItemVisitor, ItemsListVisitor
	...
	public AccessArea parseStmt(String statement) throws JSQLParserException  {
		...
		//jSQL parser uses multiple constructs, first a Statement, this can then be a select, update, create etc. statement
		Select selectStatement = (Select) stmt; //We are only interested in select statements
		...
		//SELECT clause as last one since this has no influence on the other constraints and is connected by conjunction instead of disjunction
	...
	public AccessArea parseSelect(List<SelectItem> select) throws JSQLParserException  {
		...
		while (it.hasNext()) {
			...
			if(!(item instanceof AllColumns || item instanceof AllTableColumns)){
				//A Function can also mean new relations and constraints if it is an sdss special function. transform it via our converter
				if(((SelectExpressionItem) item).getExpression() instanceof Function){
					Converter converter = new Converter();
					AccessArea tempAccessArea = converter.convertPredicate(((SelectExpressionItem) item).getExpression());
					if(!(tempAccessArea.getWhere() instanceof BinaryExpression)) tempAccessArea.setWhere(null);
					selectAccessArea = unionAccessAreas(tempAccessArea, selectAccessArea);
				}
				//if the current item contains "select" i.e. it is a subquery, then parse the subquery with the parseStmt from 'this' class and union the access areas
	...
	//Get the relations and any subquery and sdss special function from the FROM clause
	public AccessArea parseFrom(FromItem from, List<Join> joins) throws JSQLParserException  {
		...
		if(from instanceof FunctionItem) {//Get SDSS special functions, transform via converter
			Converter converter = new Converter();
			AccessArea tempArea = converter.convertPredicate(((FunctionItem)from).getFunction());
			if(tempArea.getFrom() != null) tables.addAll(tempArea.getFrom());
			if(where == null) {
				if(tempArea.getWhere() instanceof BooleanValue) {
					if(((BooleanValue)tempArea.getWhere()).getValue() == true) {}
					else where = tempArea.getWhere(); 
				} else where = tempArea.getWhere(); 
			}
		}
			...
			if(!(from instanceof SubJoin) && !(from instanceof SubSelect)) {
				...
				if(fromAccessArea.getFrom().isEmpty() && fromAccessArea.getWhere() instanceof NullValue) fromAccessArea = subJoinArea;
				else {
				...
				if(fromAccessArea.getFrom().isEmpty() && fromAccessArea.getWhere() instanceof NullValue) fromAccessArea = subSelectArea;
				else {
		...
		if(joins != null) {
			...
			while (it.hasNext()) {
				join = it.next();
				//Each JOIN has a left side and a right side, transform with converter, always check if returned constraints are Boolean values, we need to handle them
				if(join.getRightItem() instanceof FunctionItem) {
					Converter converter = new Converter();
					AccessArea tempArea = converter.convertPredicate(((FunctionItem)join.getRightItem()).getFunction());
					if(tempArea.getFrom() != null) tables.addAll(tempArea.getFrom());
					if(where == null) {
						if(tempArea.getWhere() instanceof BooleanValue) {
							if(((BooleanValue)tempArea.getWhere()).getValue() == true) {}
					...
					if(!(join.getRightItem() instanceof SubJoin) && !(join.getRightItem() instanceof SubSelect)) {
						...
						AccessArea subSelectArea = parseStmt(((SubSelect)join.getRightItem()).getSelectBody().toString());
						if(fromAccessArea.getFrom().isEmpty() && fromAccessArea.getWhere() instanceof NullValue) fromAccessArea = subSelectArea;
						else {
							fromAccessArea = intersectAccessAreas(subSelectArea,fromAccessArea);
						}
				...
				if(join.getOnExpression() != null && !join.isFull() && !join.isLeft() && !join.isRight()) {
					...
					if(tempArea.getFrom() != null) tables.addAll(tempArea.getFrom());
					if(where == null) where = tempArea.getWhere();
					else if(tempArea.getWhere() != null) {
						if(!(tempArea.getWhere() instanceof BooleanValue)) {
							andExpression = new AndExpression(null,null);
							andExpression.setLeftExpression(where);
		...
		//Either merge the multiple relations and constraints if they exist, or return just the one
		if(fromAccessArea.getFrom().isEmpty() && fromAccessArea.getWhere() instanceof NullValue) fromAccessArea = new AccessArea(tables, where);
		else {
	...
	public AccessArea unionAccessAreas(AccessArea tempAccessArea, AccessArea accessArea){
		...
		if(!(tempAccessArea.getWhere() instanceof NullValue || tempAccessArea.getWhere()==null)) {
			if(tempAccessArea.getWhere() instanceof BooleanValue && ((BooleanValue)tempAccessArea.getWhere()).getValue()) accessArea.setWhere(tempAccessArea.getWhere());
			else if(tempAccessArea.getWhere() instanceof BooleanValue && !((BooleanValue)tempAccessArea.getWhere()).getValue()) {
				if(accessArea.getWhere() == null || accessArea.getWhere() instanceof NullValue) accessArea.setWhere(tempAccessArea.getWhere());
				else accessArea.setWhere(accessArea.getWhere());
			}
			else if(!(accessArea.getWhere() instanceof NullValue || accessArea.getWhere()==null)) {
				if(accessArea.getWhere() instanceof BooleanValue && !((BooleanValue)accessArea.getWhere()).getValue()) accessArea.setWhere(tempAccessArea.getWhere());
				else if(accessArea.getWhere() instanceof BooleanValue && ((BooleanValue)accessArea.getWhere()).getValue()) accessArea.setWhere(accessArea.getWhere());
				else {
					if(accessArea.getWhere() instanceof Parenthesis || !(accessArea.getWhere() instanceof OrExpression || accessArea.getWhere() instanceof AndExpression)) tempExpr.setLeftExpression(accessArea.getWhere());
					else { 
			...
			} else { accessArea.setWhere(tempAccessArea.getWhere()); }
		}
		return accessArea;
	}
		...
		if(!(tempAccessArea.getWhere() instanceof NullValue || tempAccessArea.getWhere()==null)) {
			if(tempAccessArea.getWhere() instanceof BooleanValue && !((BooleanValue)tempAccessArea.getWhere()).getValue()) accessArea.setWhere(tempAccessArea.getWhere());
				...
				if(accessArea.getWhere() instanceof BooleanValue && ((BooleanValue)accessArea.getWhere()).getValue()) accessArea.setWhere(tempAccessArea.getWhere());
					...
					if(accessArea.getWhere() instanceof Parenthesis || !(accessArea.getWhere() instanceof OrExpression)) tempExpr.setLeftExpression(accessArea.getWhere());
					else { 
						Parenthesis tempParenthesis = new Parenthesis();
						tempParenthesis.setExpression(accessArea.getWhere());
						tempExpr.setLeftExpression(tempParenthesis);
