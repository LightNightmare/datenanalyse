		...
		if (clmnpropertySize >= 4)
		{
            ...
            case "DictionaryField":  
            {
    			...
    			while (i < clmnpropertySize)
    			{
    				...
    				((DictionaryField)Distribution).AddValue(columnValue, new ValueState(valueCount, accumulateCount, columnValue));
    				i++;
    			}
            }
                     break;
            ...
            case "DistributedFieldWithEmissions":  
            {
    			...
    			while (i < clmnpropertySize)
    			{
    				...
    				Long valueCount = Long.parseLong(vals2[1]);
    				Long accumulateCount = Long.parseLong(vals2[1]);
    				((DistributedFieldWithEmissions)Distribution).AddValue(columnValue, new ValueState(valueCount, accumulateCount, columnValue));
    				i++;
    			}
            }
                     break;
